...
podman pull $$registry$$python:3.14-debian13
```
...
```bash no-run-button no-copy-button
podman run --rm $$registry$$python:3.14-debian13 python -c 'print("Hello, Hardened Image!")'
```
...
```bash no-run-button no-copy-button
podman run --rm -it $$registry$$python:3.14-debian13 sh
```
...
::variableDefinition[host]{prompt="What is a website only accessible with your corporate issued TLS certificate?"}

```bash no-run-button no-copy-button
echo "Q" | openssl s_client -connect $$host$$:443 -showcerts 2>/dev/null | openssl x509 -noout -issuer
```

Try to access an internal website from this hardened Python image:

```bash no-run-button no-copy-button
podman run --rm $$registry$$python:3.14-debian13 python -c 'import urllib.request; print(urllib.request.urlopen("https://$$host$$", timeout=10).status)'
```
